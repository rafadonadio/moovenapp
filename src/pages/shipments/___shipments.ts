...
export class ShipmentsPage implements OnInit{
    ...
    // flag to use in view
    // to show know if user auth has been checked
        ...
        this.viewCtrl.willEnter.subscribe( () => {
            ...
            this.isUserGranted()
                .then((result:any) => {
                    ...
                    this.userUnchecked = false;
                    this.sectionEnabled = result.enabled;
                    this.operatorActivated = result.active; 
        ...
        return new Promise((resolve, reject) => {
            this.users.reloadUser()
                ...
                .then((snapshot) => {
                    ...
                    if(account.hasOwnProperty('operator')) {
                        steps.active = account.operator.active;
                        steps.enabled = account.operator.enabled;
                    }else{
                        steps.error = true;
